...
<div id="app">
  <v-app>
  <v-container>
    <h2 v-cloak>Listado de Captioners</h2>
    <v-layout>
      <v-flex>
        <v-dialog v-model="dialog" max-width="500px">
          <v-btn v-cloak slot="activator" color="222222" dark class="mb-2">Crear captioner</v-btn>
          <v-card>
            <v-card-title>
...
new Vue({
  ...
  methods: {
    ...
    deleteItem(item){
      Swal({
        title: '¿Estás seguro?',
        text: "¡El captioner será eliminado para siempre! ¡y con él todos los errores relacionados!",
        type: 'warning',
        ...
        if (result.value) {
          ...
          .then(response => {
            if(response)
            {
              Swal(
                '¡Eliminado!',
                'El captioner ha sido eliminado.',
                'success'
              ).then(response => {
                this.load();
              })
  ...
  computed: {
    formTitle () {
      return this.editedIndex === -1 ? 'Nuevo captioner' : 'Editar captioner'
    },
    rutErrors() {
      const errors = [];
    ...
    canSubmit() {
      return this.editedItem.name.length > 0 && this.rutErrors.length == 0;
    }
  },
  watch: {
    dialog (val) {
